...
Bool_t TArtEventSource::BuildEvent(TGo4EventElement* dest)
{
   ...
   // Yassid 24102013 Batch mode root file creation (Remove comment on both lines below)
   
   // if (!estore->GetNextEvent()) throw TGo4EventEndException(this);
   
   //return kTRUE;

   // For pure online just remove comment on this one Yassid
    return estore->GetNextEvent();  
